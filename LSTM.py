...
def save_model(model_wrapper, model_name):
    """
    ...
    """
    # 保存模型到 HDF5 文件
    model_wrapper.model.save(f"{model_name}.h5")
...
model = KerasRegressor(build_fn=build_model, verbose=1)  # 使用KerasRegressor包装器来包装模型
...
# load
model = load_saved_model("my_lstm_model.h5")
# 模型效果验证
